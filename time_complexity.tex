...
For each triple pattern that does not match the shape body ($Q_{s\mathrm{body}}$), the algorithm iterates over all union branches in $Q_{sunion}$, making at most $n_{sunion}$ recursive calls.
...
After the first execution, the worst-case scenario becomes one in which the triple patterns matches a pattern in $Q_{s\mathrm{body}}$, and the condition $O(tp) \in S_{star}(Q_{starG_i})$ holds.
...
$$
O\left(\sum_{i=1}^{n_{qstar}} n_{sunion} \times n_{tp_{Q_{star_i}}}\right) =O\left(n_{sunion} \times \sum_{i=1}^{n_{qstar}} n_{tp_{Q_{star_i}}}\right) = O(n_{sunion} \times n_{tp})
$$

The time complexity can alternatively be expressed as
\begin{equation}
O(n_{sunion} \times n_{tp_{Q_{star_{max}}}}),
\end{equation}
...
\begin{equation}
O(n_{sunion} \times n_{tp_{max}} \times n_{Q_{star_{max}}} \times n_{Q_{starG}}),
\end{equation}
